...
/**
 * deepseek-v4-* (y variantes OpenAI-compatibles) soportan tool_calls nativos,
 * igual que OpenAI. Se reutiliza el patrón del OpenAIAdapter pero apuntando
 * el SDK al endpoint de DeepSeek y leyendo la key desde DEEPSEEK_API_KEY para
 * que el SDK nunca resuelva la key a otro proveedor.
 */
@Injectable()
export class DeepSeekAdapter implements LLMProviderPort {
  ...
  async chat(req: ChatRequest): Promise<ChatResponse> {
    ...
    try {
      ...
      if (error?.status === 400) {
        this.logger.warn(`[DeepSeek] 400 from API, retrying once: ${error.message}`);
        const response = await this.client.chat.completions.create(
          {
            model: this.model,
            max_tokens: req.maxTokens,
  ...
  private parseResponse(
    ...
    if (!choice) {
      throw new Error('DeepSeek returned no choices (possible content filter or safety block)');
    }
    ...
    this.logger.log(
      `[DeepSeek] ${this.model} | ${latencyMs}ms | in=${usage.inputTokens} out=${usage.outputTokens} | $${cost.totalUsd.toFixed(6)}`,
    );
